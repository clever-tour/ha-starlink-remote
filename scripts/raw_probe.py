...
def main():
    ...
    # Request for Router-0100000000000000008B65AD
    router_id = "Router-0100000000000000008B65AD"
    req = Request(target_id=router_id, get_status=GetStatusRequest())
    
    # Wrap in gRPC-Web frame (5-byte header: 0x00 + 4-byte length)
    serialized = req.SerializeToString()
    frame = b'\x00' + len(serialized).to_bytes(4, 'big') + serialized
    
    print(f"[*] Sending raw HTTP probe to {url}...")
    print(f"[*] Target ID: {router_id}")
    
    try:
        resp = httpx.post(url, headers=headers, content=frame, timeout=10.0)
        
        print(f"[DEBUG] HTTP Status: {resp.status_code}")
        print(f"[DEBUG] gRPC Headers: {json.dumps(dict(resp.headers), indent=2)}")
        
        if resp.status_code == 200:
            # Check for gRPC error in trailers/headers
            grpc_msg = resp.headers.get("grpc-message")
            if grpc_msg and grpc_msg != "OK":
                print(f"[-] Backend rejected ID: {grpc_msg}")
            else:
                print("[+] SUCCESS! The backend accepted the request.")
                print("[+] Raw body sample (hex):", resp.content[:20].hex())
        else:
