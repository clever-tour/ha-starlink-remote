...
def run_test():
    ...
    print("[*] Attempting Session Boost via Alignment Page...")
    try:
        r = client.get("https://www.starlink.com/account/home", headers=headers)
        print(f"  [HTTP] Dashboard Status: {r.status_code}")
        # Search for any redirected URL
        print(f"  [INFO] Final URL: {r.url}")
        
        r2 = client.get("https://api.starlink.com/auth-rp/auth/user", headers=headers)
        print(f"  [HTTP] Auth-RP Status: {r2.status_code}")
        
        # Test if we can now hit discovery
        r3 = client.get("https://api.starlink.com/webagg/v2/accounts/service-lines", headers=headers)
        print(f"  [HTTP] Discovery Status after Boost: {r3.status_code}")
