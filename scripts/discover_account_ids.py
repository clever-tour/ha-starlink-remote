...
def run_test():
    ...
    print("[*] Discovering Hardware for Account ACC-5147187...")
    # Hit the service-lines API which lists all hardware for the account
    try:
        r = client.get("https://api.starlink.com/webagg/v2/accounts/service-lines", headers=headers)
        print(f"  [HTTP] Status: {r.status_code}")
        if r.status_code == 200:
            print("  [SUCCESS] Received Hardware List:")
            data = r.json()
            for line in data.get('content', {}).get('results', []):
                for ut in line.get('userTerminals', []):
                    print(f"    - Dish ID: ut{ut.get('userTerminalId')}")
                    for router in ut.get('routers', []):
                        print(f"    - Router ID: Router-{router.get('routerId')}")
        else:
            print(f"  [-] Error: Backend returned {r.status_code}. Session is likely restricted.")
