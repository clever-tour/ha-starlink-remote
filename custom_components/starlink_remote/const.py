DOMAIN = 'starlink_remote'
CONF_COOKIE = 'cookie'
CONF_ROUTER_ID = 'router_id'
CONF_SCAN_INTERVAL = 'scan_interval'
CONF_NAME = 'name'
CONF_COOKIE_DIR = 'cookie_dir'
DEFAULT_SCAN_INTERVAL = 60
DEFAULT_COOKIE_DIR = '/config/.starlink_cookies'
DATA_DISH_STATUS = 'dish_status'
DATA_DISH_HISTORY = 'dish_history'
DATA_WIFI_CLIENTS = 'wifi_clients'
DATA_SERVICE_LINES = 'service_lines'
DATA_USAGE = 'data_usage'
STARLINK_WEB_SERVICE_LINES = 'https://api.starlink.com/webagg/v2/accounts/service-lines'
STARLINK_API_URL = 'https://api2.starlink.com/SpaceX.API.Device.Device/Handle'
STARLINK_AUTH_URL = 'https://api.starlink.com/auth-rp/auth/user'
